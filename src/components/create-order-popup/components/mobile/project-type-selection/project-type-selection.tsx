...
import { ProjectTypes, projectTypeOptions, subjects } from "@/utils/popupOrder/utils";
...
const TextArea = ({ workTheme }: { workTheme: string }) => {
  ...
  return (
    <div className={styles.text_area_container}>
      <textarea ref={textAreaRef} className={styles.text_area} value={textArea} onChange={onChange} />
      <label className={styles.text_area_label}>{textArea.length}/1000</label>
    </div>
  );
};

const WorkThemeSubject = ({ workTheme, subject }: { workTheme: string; subject: string }) => {
  ...
  return (
    <div className={styles.project_type} style={{ flex: "1", justifyContent: "space-between" }}>
...
export const ProjectTypeSelection = () => {
  ...
  return (
    <div className={styles.project_type}>
      ...
      <WorkThemeSubject workTheme={workTheme} subject={subject} />
      {projectType != null && (workTheme !== "" || subject !== "") && (
        <StageControls forwardButton={() => dispatch(setStage("date"))} />
      )}
    </div>
  );
};
